...
export default function Header() {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!text.includes("==========") || !text.match(/\(.+\)/)) {
      showToast(
        "Invalid file format. Please upload a valid Kindle My Clippings.txt file."
      );
      return;
    }
    ...
    const encryptedBooks = await Promise.all(
      parsedBooks.map(async (book) => {
        const encryptedTitle = await encryptWithKey(encryptionKey, book.title);
        const encryptedAuthor = await encryptWithKey(
          encryptionKey,
          book.author
        );
        const encryptedClippings = await Promise.all(
          book.clippings.map(async (clip) => ({
            ...clip,
